...
std::vector<float> clamp_at_boundaries(float requested_j1, float requested_j2){
    // Define clamped joint angle and assign them to the requested ones
    ...
    // Get joins min and max parameters
    ...
    ROS_INFO("min_j1: %1.2f", min_j1);
    ROS_INFO("max_j1: %1.2f", max_j1);
    ROS_INFO("min_j2: %1.2f", min_j2);
    ROS_INFO("max_j2: %1.2f", max_j2);

    // Check if joint 1 falls in the same zone, otherwise clamp it
    ...
    // Check if joint 2 falls in the same zone, otherwise clamp it
    if(requested_j2 < min_j2 || requested_j2 > max_j2){
        clamped_j2 = std::min(std::max(requested_j2, min_j2), max_j2);
        ROS_WARN("j2 is out of bounds, valid range (%1.2f, %1.2f), clamping to %1.2f", min_j1, max_j1, clamped_j1);
    }
...
bool handle_safe_move_request(simple_arm::GoToPosition::Request& req, simple_arm::GoToPosition::Response& res){
    ...
    //Check if requested joint angles are in the safe zone, otherwise clamp
    ...
    // Wait 3 seconds for the arm to settle
    ros::Duration(3).sleep();

    //Return a response message
    res.msg_feedback = "Joint angles set to - j1: " + std::to_string(joints_angles[0]) + ", j2: " + std::to_string(joints_angles[1]);
