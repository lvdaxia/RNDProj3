...
// Define global vector of joints last position, moving state of the arm, and the client that can request the services
...
// This call back function continuously executes and reads the arm joint angles position
...
void look_away_callback(const sensor_msgs::Image img){
    ...
    // Loop through each pixel in the image and check if it's equal to the first pixel
    ROS_INFO_STREAM("look_away_callback called");
...
int main(int argc, char** argv){
    ...
    // Subscribe to rgb_camera/image_raw topic to read the image data inside the look_away callback function
